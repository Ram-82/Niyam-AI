...
document.addEventListener('DOMContentLoaded', function () {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: function(info, successCallback, failureCallback) {
            // Fetch real deadlines from TDS + ROC + GST endpoints
            fetchCalendarEvents().then(successCallback).catch(() => successCallback([]));
        },
...
async function fetchCalendarEvents() {
    ...
    if (!NiyamAuth.isAuthenticated()) return events;

    // Fetch TDS deadlines
    try {
        const tdsResp = await NiyamAuth.niyamFetch(`${API_URL}/tds/deadlines`);
        const tdsData = await tdsResp.json();
        if (tdsData.success && tdsData.data && tdsData.data.deadlines) {
            for (const dl of tdsData.data.deadlines) {
                events.push({
                    title: dl.subtype || 'TDS',
                    start: dl.due_date,
                    color: dl.status === 'completed' ? '#94a3b8' : typeColors.tds,
                    extendedProps: {
                        description: dl.description || dl.subtype,
                        status: dl.status,
                    },
                });
            }
        }
    } catch (e) { console.warn('Calendar: TDS fetch failed', e); }

    // Fetch ROC deadlines
    try {
        const rocResp = await NiyamAuth.niyamFetch(`${API_URL}/roc/deadlines`);
        const rocData = await rocResp.json();
        if (rocData.success && rocData.data && rocData.data.deadlines) {
            for (const dl of rocData.data.deadlines) {
                events.push({
                    title: dl.subtype || 'ROC',
                    start: dl.due_date,
                    color: dl.status === 'completed' ? '#94a3b8' : typeColors.roc,
                    extendedProps: {
                        description: dl.description || dl.subtype,
                        status: dl.status,
                    },
    ...
    } catch (e) { console.warn('Calendar: ROC fetch failed', e); }

    // Add GST deadlines from dashboard data (generated by rules engine)
    // These come from the dashboard summary timeline
    try {
        const dashResp = await NiyamAuth.niyamFetch(`${API_URL}/dashboard/summary`);
        const dashData = await dashResp.json();
        if (dashData.success && dashData.data && dashData.data.risk_timeline) {
            for (const item of dashData.data.risk_timeline) {
                if (item.due_date && item.category && !events.find(e => e.start === item.due_date && e.title === (item.title || '').split(' ')[0])) {
                    events.push({
                        title: (item.title || item.category).split(' is ')[0].split(' due ')[0],
                        start: item.due_date,
                        color: item.severity === 'critical' ? '#ef4444' : typeColors[item.category] || '#6b7280',
                        extendedProps: {
                            description: item.action_required || item.title,
    ...
    } catch (e) { console.warn('Calendar: dashboard fetch failed', e); }

    // Populate "Upcoming This Week" sidebar
...
function populateUpcomingEvents(events) {
    ...
    const upcoming = events
        .filter(e => e.start >= todayStr && e.start <= weekEndStr && (!e.extendedProps || e.extendedProps.status !== 'completed'))
        .sort((a, b) => a.start.localeCompare(b.start))
        .slice(0, 5);

    if (upcoming.length === 0) {
        container.innerHTML = '<p style="font-size:0.85rem; color:var(--text-light);">No deadlines this week. You\'re on track!</p>';
        return;
    }
